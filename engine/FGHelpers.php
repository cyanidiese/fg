...
function fg_is_active_member_activated(){
    return in_array( 'activemember360/activemember360.php', (array) get_option( 'active_plugins', array() ) );
}

function fg_get_known_cities() {
    return array(
        1   => 'Albany',
        2   => 'Alexandria',
        3   => 'Allentown',
...
function fg_get_known_and_saved_cities() {

    global $wpdb;

    $autocomplete = array();

    $query = "SELECT distinct(`meta_value`) FROM `{$wpdb->postmeta}` WHERE `meta_key` = 'fg_city' ORDER BY `meta_value` ASC";
